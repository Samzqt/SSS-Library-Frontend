...
async function updateCounts() {
  try {
    const activeBooksCount = await getActiveBooksCount();
    const activeAuthorsCount = await getActiveAuthorsCount();
    const activeGenresCount = await getActiveGenresCount();

    // Update the counts in the HTML
    document.getElementById("active-books-count").innerText = activeBooksCount;
    document.getElementById("active-authors-count").innerText = activeAuthorsCount;
    document.getElementById("active-genres-count").innerText = activeGenresCount;
  } catch (error) {
    console.error("Error updating counts:", error);
  }
}

// Call this function in your initPage function after loading the page
async function initPage() {
  await Promise.all([getAuthorsDropdown(), getGenresDropdown()]);
  ...
  // Count and display the number of books, authors, and genres
  countAndDisplay();
